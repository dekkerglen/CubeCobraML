...
The demo server reads two artifacts that are **not** regenerated by `scripts/convert.sh`:
...
- `demo/server/embeddings.json` — precomputed encoder outputs for every oracle, used by the **Synergy** page. This file is **stale after every retrain** and must be regenerated against the freshly converted `tfjs_model/encoder/`. Symptom of a stale file: the Synergy page returns "No results yet" for any card whose index exceeds the embedding count, because the server throws on `undefined` lookups.

To regenerate after a retrain + convert:

```bash
cd demo/server
cp ../../raw_data/indexToOracleMap.json .   # only if the catalog changed
node compile_embeddings.js                  # rebuilds embeddings.json from tfjs_model/encoder
```
