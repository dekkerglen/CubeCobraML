...
class DataGenerator(Sequence):
    def __init__(
        ...
        epsilon = 1
        self.card_correlations_y = self.card_correlations_y / (
            self.card_correlations_y.sum(axis=1, keepdims=True) + epsilon
        )
        ...
        self.corr_batch_size = 1  # (len(self.corr_indices) * corr_multiplier)// self.n_batches
        self.cube_batch_size = 1  # (len(self.x_cubes_files) * cube_multiplier) // self.n_batches
        self.deck_batch_size = 1  # len(self.x_decks_files) // self.n_batches
        self.pick_batch_size = 1  # len(self.x_picks_files) // self.n_batches
    ...
    def __getitem__(self, batch_number):
        ...
        if corr_start + self.corr_batch_size < self.num_cards:
            ...
            corr_indeces = np.concatenate(
                (
                    self.corr_indices[corr_start:],
                    self.corr_indices[: self.corr_batch_size - (self.num_cards - corr_start)],
                )
            )
