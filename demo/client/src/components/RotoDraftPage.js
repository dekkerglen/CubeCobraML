...
function RotoDraftPage() {
  ...
  const [cubeText, setCubeText] = useLocalStorage('rotocubeinput', '');
  const [landPctText, setLandPctText] = useLocalStorage('rotolandpct', '0');
  const [nonlandPctText, setNonlandPctText] = useLocalStorage('rotononlandpct', '0');
  ...
  const fetchPicks = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch('/api/rotodraft', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ pool: poolNames, cube: cubeNames, landPct, nonlandPct }),
      });
  ...
  }, [poolText, cubeText, landPctText, nonlandPctText]);

  return (
    <Row className="h-100 g-3">
      <Col md="4" className="h-100 overflow-auto">
        <Label className="fw-bold mb-1">Pool</Label>
        <Input type="textarea" rows="8" value={poolText} onChange={e => setPoolText(e.target.value)} />
